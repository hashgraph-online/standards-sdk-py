...
class AsyncRegistryBrokerClient:
    ...
    async def quote_skill_publish_preview(
        self,
        *,
        file_count: int,
        total_bytes: int,
        ...
        raw = await self.call_operation("quote_skill_publish_preview", body=payload)
        return self._parse_model(raw, SkillQuotePreviewResponse)

    async def get_skill_conversion_signals_by_repo(
        self,
        *,
        repo: str,
        skill_dir: str,
        ...
        if ref is not None:
            query["ref"] = ref
        raw = await self.call_operation("get_skill_conversion_signals_by_repo", query=query)
    ...
    async def get_skill_install(self, skill_ref: str) -> SkillInstallResponse:
        raw = await self.call_operation(
            "get_skill_install",
